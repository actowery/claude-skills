...
def handles_to_query(cfg, include_team, author_override=None):
    """Return list of (github_username, display_name) tuples to search.

    If author_override is set, returns ONLY that handle (with display name
    looked up from team.members[] / manager / peers[] if present, else empty).
    This is the 1:1 prep use case — narrow to one specific person.

    Otherwise, always includes the primary user. If include_team is True and
    the config has team.members[], each member with a github_username is
    appended. Missing handles produce a warning and are skipped (not fatal).
    """
    if author_override:
        # 1:1 prep mode — single target
        display = ""
        # Look up display name from team / manager / peers
        for m in (cfg.get("team") or {}).get("members") or []:
            if m.get("github_username") == author_override:
                display = m.get("display_name", "")
                break
        if not display:
            mgr = cfg.get("manager") or {}
            if mgr.get("github_username") == author_override:
                display = mgr.get("display_name", "")
        if not display:
            for p in cfg.get("peers") or []:
                if p.get("github_username") == author_override:
                    display = p.get("display_name", "")
                    break
        return [(author_override, display)]
...
def cmd_prs(args):
    ...
    pairs = handles_to_query(cfg, args.include_team, getattr(args, "author", None))
    seen_urls = set()
    normalized = []
...
def main():
    ...
    pr.add_argument("--author", default=None,
                    help="Override and search for ONLY this github_username (1:1 prep mode)")
    ...
    pc.add_argument("--include-team", action="store_true",
                    help="Fan out to config.team.members[].github_username")
    pc.add_argument("--author", default=None,
                    help="Override and search for ONLY this github_username (1:1 prep mode)")
